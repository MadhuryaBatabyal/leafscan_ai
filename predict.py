...
def _detect_num_classes(path: str, stage: str) -> int:
    """
    Reads the saved state_dict and extracts num_classes from the final layer.
    Keys confirmed by direct inspection of the .pth files:
      dryness/pest → EfficientNet-B0 → "classifier.1.weight"
      disease      → ResNet-50       → "fc.weight"
    weights_only=False is required because these files contain
    torchvision storage classes not on the safe allowlist.
    """
    # suppress the FutureWarning about weights_only default changing
    import warnings
    with warnings.catch_warnings():
        warnings.simplefilter("ignore")
        state = torch.load(path, map_location="cpu", weights_only=False)

    key = "fc.weight" if stage == "disease" else "classifier.1.weight"
    if key not in state:
        raise KeyError(
            f"Expected key {key!r} not found in {path}.\n"
            f"Available keys (last 5): {list(state.keys())[-5:]}"
        )
