...
def _download_file_from_gdrive(file_id: str, dest_path: str):
    ...
    Handles the large-file confirmation page that Drive shows for files >100MB.
    """
    ...
    # Check if Drive is asking for confirmation (large file warning)
    token = None
    ...
    # If confirmation needed, resend with the token
    if token:
    ...
    # Write the file in chunks (avoids memory issues with large files)
